...
export function PaywallProvider({ children }: { children: React.ReactNode }) {
  ...
  const open = useCallback((r = "Débloque tout OUVATU") => setReason(r), []);
  return (
    <PaywallContext.Provider value={open}>
      ...
      <Sheet open={reason !== null} onClose={() => setReason(null)} hideClose>
        <div className="text-center">
          ...
          <p className="mt-1 text-muted">Passe à Premium et profite de tout ce qu&apos;OUVATU détecte.</p>
          ...
          <p className="mt-3 text-xs text-subtle">
            Dès {formatPrice(PREMIUM_OFFERS.week.amountCents)}/semaine · Sans engagement
          </p>
